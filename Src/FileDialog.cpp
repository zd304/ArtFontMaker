...
bool FileDialog::DoModal()
{
	...
	if (ImGui::BeginPopupModal(STU(dlgName), NULL, ImGuiWindowFlags_AlwaysAutoResize))
	{
		ImGui::Text(STU("路径"));
		ImGui::SameLine();
		...
		ImGui::SameLine(0, 16);
		ImGui::PushID(Global::mItemID++);
		if (ImGui::Button(STU("向上")))
		{
			std::string s = directory;
			if (s.size() > 0)
			{
				EditorUtility::StringReplace(s, "/", "\\");
		...
		if (ImGui::Button(STU("新建")))
		{
			if (directory[0] != '\0')
			{
				memset(szFolder, 0, 128);
				ImGui::OpenPopup(STU("新建新建文件夹"));
			}
		}
		
		if (ImGui::BeginPopupModal(STU("新建新建文件夹"), NULL, ImGuiWindowFlags_AlwaysAutoResize))
		{
			ImGui::Text(STU("文件夹名字"));
			ImGui::SameLine();
			ImGui::InputText(STU("(中文会乱码)"), szFolder, 128);
			ImGui::Spacing();
			ImGui::Separator();
			if (ImGui::Button(STU("确定")) && szFolder[0] != '\0')
			{
		...
		else
		{
			for (size_t i = 0; i < folders.size(); ++i)
			{
				...
				ImGui::SameLine();
				ImGui::PushID(Global::mItemID++);
				if (ImGui::Selectable(STU(folder), selFolderIndex == i, ImGuiSelectableFlags_AllowDoubleClick, ImVec2(0, 32)))
				{
					if (ImGui::IsMouseDoubleClicked(0))
					{
						path = path + folder + "\\";
			...
			for (size_t i = 0; i < files.size() && mUsage != eFileDialogUsage_OpenFolder; ++i)
			{
				...
				ImGui::SameLine();
				ImGui::PushID(Global::mItemID++);
				if (ImGui::Selectable(STU(file), selFileIndex >= 0 && selFileIndex == i, 0, ImVec2(0, 32)))
				{
					selFileIndex = i;
					fileName = file;
					memset(cur_filename, 0, MAX_PATH);
		...
		{
			if (ImGui::Button(STU("打开"), ImVec2(80, 40)))
			{
				if (fileName.size() == 0)
				{
		...
		else if (mUsage == eFileDialogUsage_SaveFile)
		{
			...
			bool close = false;
			if (ImGui::Button(STU("保存"), ImVec2(80, 40)) && directory[0] != '\0')
			{
				if (fileName.size() == 0)
				{
			...
			if (ImGui::BeginPopupModal(STU("提醒"), NULL, ImGuiWindowFlags_AlwaysAutoResize))
			{
				ImGui::Text(STU("文件已经存在，是否替换"));
				ImGui::Separator();
				if (ImGui::Button(STU("是"), ImVec2(60, 24)))
				{
					ImGui::CloseCurrentPopup();
					rst = true;
					close = true;
				...
				ImGui::SameLine();
				if (ImGui::Button(STU("否"), ImVec2(60, 24)))
				{
					fileName = "";
					ImGui::CloseCurrentPopup();
				}
		...
		{
			if (ImGui::Button(STU("选择文件夹"), ImVec2(80, 40)) && directory[0] != '\0')
			{
				if (fileName.size() == 0)
				{
					fileName = curFileName;
		...
		if (ImGui::Button(STU("取消"), ImVec2(80, 40)))
		{
			ImGui::CloseCurrentPopup();
		}
